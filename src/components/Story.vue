<template>
  <div class="chartArea" >
  <h1>{{this.name }}</h1>
    <h3><router-link :to="'/history/'+this.ID">See run history and score!</router-link></h3>
    <h3 v-if="this.gitHub != null">GitHub: {{this.gitHub}}</h3>
    <pre><p v-if="challenge!=null">{{challenge}}</p></pre>
    <br>
    <button @click="this.toggleSpoiler" v-if="!this.spoilerModal">Show Pseudocode & Methodology</button>
    <button @click="this.toggleSpoiler" v-if="this.spoilerModal">Collapse</button>
